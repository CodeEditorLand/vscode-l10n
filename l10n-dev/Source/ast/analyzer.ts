...
import {
	getTQuery,
	IAlternativeVariableNames,
	importOrRequireQuery,
} from "./queries";
...
export class ScriptAnalyzer {
	...
	#getCommentsFromMatch(match: QueryMatch): string[] {
		...
		return commentCapture.node.children
			...
			.map((c) =>
				this.#getUnquotedString(
					c.type === "string"
						? c.text
						: this.#getTemplateValueFromTemplateRawValue(c.text),
				),
			);
	}
	...
	#getImportDetails(match: QueryMatch): IAlternativeVariableNames {
		...
		if (importArg) {
			const namespace = this.#getStringFromMatch(
				match,
				"namespace",
				false,
			);
	...
	async analyze({
		extension,
		contents,
	}: IScriptFile): Promise<l10nJsonFormat> {
