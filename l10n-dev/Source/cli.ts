...
import {
	getL10nAzureLocalized,
	getL10nFilesFromXlf,
	getL10nJson,
	getL10nPseudoLocalized,
	getL10nXlf,
	l10nJsonFormat,
} from "./main";
	...
	.command(
		...
		(yargs) => {
			...
			yargs.option("language", {
				...
				default: "en",
				describe:
					"The source language that will be written to the XLF file.",
			});
		},
		function (argv) {
			l10nGenerateXlf(
	...
	.command(
		...
		(yargs) => {
			...
			yargs.option("outDir", {
				...
				default: ".",
				describe:
					"Output directory that will contain the l10n.<language>.json files",
			});
		},
		async function (argv) {
...
export async function l10nExportStrings(
	paths: string[],
	outDir?: string,
): Promise<void> {
	...
	const matches = glob.sync(
		paths.map((p) =>
			/\.(ts|tsx|js|jsx)$/.test(p)
				? p
				: path.posix.join(p, "{,**}", "*.{ts,tsx,js,jsx}"),
		),
		GLOB_DEFAULTS,
	);
	...
	if (packageJSON) {
		if (outDir) {
			if (
				!packageJSON.l10n ||
				path.resolve(packageJSON.l10n) !== path.resolve(outDir)
			) {
				console.warn(
...
export function l10nGenerateXlf(
	paths: string[],
	language: string,
	outFile: string,
): void {
	...
	const matches = glob.sync(
		paths.map((p) =>
			/(\.l10n\.json|package\.nls\.json)$/.test(p)
				? p
				: path.posix.join(
						p,
						`{,!(node_modules)/**}`,
						"{*.l10n.json,package.nls.json}",
					),
		),
		GLOB_DEFAULTS,
	);
...
export async function l10nImportXlf(
	paths: string[],
	outDir: string,
): Promise<void> {
	...
	const matches = glob.sync(
		paths.map((p) =>
			/\.xlf$/.test(p)
				? p
				: path.posix.join(p, `{,!(node_modules)/**}`, "*.xlf"),
		),
		GLOB_DEFAULTS,
	);
		...
		for (const detail of details) {
			...
			writeFileSync(
				path.resolve(
					path.join(
						outDir,
						`${detail.name}.${type}.${detail.language}.json`,
					),
				),
...
export async function l10nGenerateTranslationService(
	paths: string[],
	languages: string[],
	key: string,
	region: string,
